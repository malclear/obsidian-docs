...
To make the link display text in Preview that is differnt from the the note's name, use the vertical pipe (`Shift+|`) between the file's name and the custom link text. For example: [[Internal link|Custom Link Name in Preview!]] This can be combined with linking to headers, as in [[Folding#By way of example|Example of Folding]].
